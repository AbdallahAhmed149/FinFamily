...
from db.models import User, UserRole, Wallet, Mission, MissionKind, MissionStatus, Transaction, TransactionType
from services.scoring import _redemption_spend_since
...
def build_family_insights(db: Session, parent: User) -> List[dict]:
    ...
    for c in children:
        ...
        if wallet.card_status.value == "frozen":
            insights.append(_insight("shield-alert", "alert", f"{c.full_name}'s card is frozen", "Unfreeze it from Family Members if this was resolved."))

        exceeded = []
        if wallet.daily_limit and _redemption_spend_since(db, wallet.id, now.replace(hour=0, minute=0, second=0, microsecond=0)) > wallet.daily_limit:
...
def build_family_activity(db: Session, parent: User, limit: int = 10) -> List[dict]:
    """آخر الأحداث الحقيقية في العيلة (مهام اتوافق عليها/اترفضت/اتبعتت، ومصروف اتبعت) — مبنية من Mission/Transaction الفعليين."""

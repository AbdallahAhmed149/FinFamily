...
export default function ChildHome() {
  ...
  useEffect(() => {
    ...
    setGreet(h < 12 ? t("child_home.greeting.morning") : h < 18 ? t("child_home.greeting.afternoon") : t("child_home.greeting.evening"));
  }, [t]);
    ...
    <div className="px-4 pt-12 relative">
        ...
        <div className="grad-navy rounded-3xl p-5 text-white shadow-premium relative overflow-hidden animate-gradient">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-white/70 font-medium">{t("child_home.hero.status")}</span>
              <Pill className="bg-white/15 text-white"><Flame className="w-3 h-3" /> {t("child_home.hero.streak", { count: userStreak })}</Pill>
            </div>
            <div className="text-lg font-extrabold font-heading">{t("child_home.hero.level", { level: userLevel, name: level.name })}</div>
            <div className="mt-3">
              <div className="flex justify-between text-xs mb-1">
                <span className="text-white/70">{userXp} {t("child_home.hero.xp")}</span>
                <span className="text-white/70">{nextLevel ? `${nextLevel.minXp} ${t("child_home.hero.pts")}` : t("child_home.hero.max")}</span>
              </div>
              <div className="h-2.5 rounded-full bg-white/15 overflow-hidden">
                <div className="h-full rounded-full grad-gold transition-all duration-700" style={{ width: `${progressPct}%` }} />
              </div>
              <div className="text-[11px] text-white/60 mt-1">{nextLevel ? t("child_home.hero.to_next", { pts: nextLevel.minXp - userXp, name: nextLevel.name }) : t("child_home.hero.max_reached")}</div>
            </div>
          </div>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={100} className="grid grid-cols-3 gap-3 mt-4">
        <div className="glass rounded-2xl p-3 text-center shadow-premium">
          ...
          <div className="text-[10px] text-muted-foreground">{t("child_home.stats.xp")}</div>
        </div>
        <div className="glass rounded-2xl p-3 text-center shadow-premium">
          <Flame className="w-5 h-5 text-red-500 mx-auto mb-1" />
          <div className="text-lg font-extrabold font-heading">{userStreak}</div>
          <div className="text-[10px] text-muted-foreground">{t("child_home.stats.streak")}</div>
        </div>
        <div className="glass rounded-2xl p-3 text-center shadow-premium">
          ...
          <div className="text-[10px] text-muted-foreground">{t("child_home.stats.coins")}</div>
        </div>
      </FadeIn>

      {/* today's mission */}
      <FadeIn delay={140}>
        <SectionTitle>{t("child_home.today_mission")}</SectionTitle>
        <div className="rounded-3xl p-5 shadow-premium relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${mission.color}, ${mission.color}cc)` }}>
          ...
          <div className="relative text-white">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[11px] bg-white/20 rounded-full px-2 py-0.5 font-semibold">{t("child_home.pts_plus", { xp: mission.xp })}</span>
            </div>
            <button onClick={() => launchGame(mission.game)} className="w-full h-12 rounded-2xl bg-white text-sm font-extrabold font-heading flex items-center justify-center gap-2 active:scale-[0.98] transition-all" style={{ color: mission.color }}>
              <Play className="w-4 h-4" /> {t("child_home.start_mission")}
            </button>
          </div>
        </div>
      </FadeIn>
      ...
      {/* play & learn */}
      <FadeIn delay={180}>
        <SectionTitle action={<button onClick={() => navigate("/child/adventure")} className="text-xs font-bold text-emerald-600 flex items-center gap-1">{t("child_home.adventure_map")} <ChevronRight className="w-3 h-3" /></button>}>{t("child_home.play_learn")}</SectionTitle>
            ...
            <button key={c.id} onClick={() => c.locked ? null : launchGame(c.game)} disabled={c.locked} className={"w-full glass rounded-2xl p-4 flex items-center gap-3 shadow-premium transition-all " + (c.locked ? "opacity-60" : "active:scale-[0.99]")}>
              ...
              <div className="flex-1 text-left">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <span className="text-[10px] font-bold text-amber-600">{t("child_home.pts_plus", { xp: c.xp })}</span>
                  {!c.locked && c.progress > 0 && <span className="text-[10px] text-muted-foreground">{t("child_home.completed", { pct: c.progress })}</span>}
                </div>
              </div>
      ...
      {/* adventure shortcuts */}
      <FadeIn delay={220}>
        <SectionTitle>{t("child_home.shortcuts.title")}</SectionTitle>
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => navigate("/child/adventure")} className="glass rounded-2xl p-4 text-left shadow-premium active:scale-95 transition-all">
            ...
            <div className="font-bold text-sm">{t("child_home.shortcuts.map.title")}</div>
            <div className="text-xs text-muted-foreground">{t("child_home.shortcuts.map.desc")}</div>
          </button>
          <button onClick={() => navigate("/child/missions")} className="glass rounded-2xl p-4 text-left shadow-premium active:scale-95 transition-all">
            ...
            <div className="font-bold text-sm">{t("child_home.shortcuts.missions.title")}</div>
            <div className="text-xs text-muted-foreground">{t("child_home.shortcuts.missions.desc")}</div>
          </button>
          <button onClick={() => navigate("/child/coach")} className="glass rounded-2xl p-4 text-left shadow-premium active:scale-95 transition-all">
            ...
            <div className="font-bold text-sm">{t("child_home.shortcuts.coach.title")}</div>
            <div className="text-xs text-muted-foreground">{t("child_home.shortcuts.coach.desc")}</div>
          </button>
          <button onClick={() => navigate("/child/leaderboard")} className="glass rounded-2xl p-4 text-left shadow-premium active:scale-95 transition-all">
            ...
            <div className="font-bold text-sm">{t("child_home.shortcuts.leaderboard.title")}</div>
            <div className="text-xs text-muted-foreground">{t("child_home.shortcuts.leaderboard.desc")}</div>
          </button>
          <button onClick={() => navigate("/child/wallet")} className="glass rounded-2xl p-4 text-left shadow-premium active:scale-95 transition-all">
            ...
            <div className="font-bold text-sm">{t("child_home.shortcuts.wallet.title")}</div>
            <div className="text-xs text-muted-foreground">{t("child_home.shortcuts.wallet.desc")}</div>
          </button>
          <button onClick={() => navigate("/child/goals")} className="glass rounded-2xl p-4 text-left shadow-premium active:scale-95 transition-all">
            ...
            <div className="font-bold text-sm">{t("child_home.shortcuts.goals.title")}</div>
            <div className="text-xs text-muted-foreground">{t("child_home.shortcuts.goals.desc")}</div>
          </button>
        </div>
      </FadeIn>

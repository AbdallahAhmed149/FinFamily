...
import { adventureWorlds, child, levels } from "@/lib/finData";
  ...
  return (
    <div className="px-4 pt-12">
      <FadeIn className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h1 className="text-xl font-extrabold font-heading">{t("adventure_map.title")}</h1>
          <div className="text-xs text-muted-foreground">{t("adventure_map.subtitle", { level: child.level, name: level.name, xp: child.xp })}</div>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={60}>
        <div className="grad-navy rounded-3xl p-4 text-white shadow-premium text-center relative overflow-hidden">
          ...
          <div className="relative text-sm">{t("adventure_map.hero_journey")}</div>
          <div className="text-xs text-white/70 mt-1">{t("adventure_map.hero_desc")}</div>
        </div>
      </FadeIn>
                ...
                <div className="relative pl-16">
                  ...
                  <button onClick={() => launch(w)} disabled={locked} className={"w-full glass rounded-2xl p-4 text-left shadow-premium transition-all " + (locked ? "opacity-60" : "active:scale-[0.99]")}>
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-[10px] font-bold text-muted-foreground">{t("adventure_map.world", { num: idx + 1 })}</div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {w.topics.map((topic) => (
                        <span key={topic} className="text-[10px] rounded-full px-2 py-0.5" style={{ background: `${w.color}12`, color: w.color }}>{topic}</span>
                      ))}
                    </div>
                    {!locked && (
                      <div className="mt-3">
                        <div className="flex justify-between text-[10px] text-muted-foreground mb-1"><span>{t("adventure_map.progress")}</span><span>{w.progress}%</span></div>
                        <ProgressBar value={w.progress} color={w.color} />
                      </div>
                    )}
                    {locked && <div className="text-[11px] text-muted-foreground mt-2 flex items-center gap-1"><MapPin className="w-3 h-3" /> {t("adventure_map.unlocks_at", { level: w.unlockLevel })}</div>}
                  </button>
                </div>
              </FadeIn>
            );
